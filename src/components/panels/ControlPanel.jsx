...
import { formatValue, mainDirectionLabel, pointTypeLabel, routeLabel } from "@/lib/domain/formatters";
...
function LayerToggle({ active, children, onClick }) {
   return (
      <button
         aria-pressed={active}
         className="inline-flex min-w-0 min-h-9 items-center justify-between gap-2 rounded-md border border-border bg-muted px-2.5 text-[0.72rem] text-foreground transition-colors hover:border-primary/70 hover:bg-primary/20 hover:text-secondary focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none max-[1180px]:min-h-10"
         onClick={onClick}
         type="button"
      >
         <span className="min-w-0 truncate">{children}</span>
         <i aria-hidden="true" className={cn("size-2 rounded-full bg-border", active && "bg-primary")} />
      </button>
   );
}
...
function ResultItem({ result, onSelect }) {
   ...
   const meta = isPoint
      ...
      : `${item.properties.id} · ${routeLabel(item.properties.route_group ?? item.properties.relation_type)} · ${formatValue(item.properties.length_km, " km")}`;
...
export default function ControlPanel({
   ...
   return (
      <aside className="flex min-h-0 flex-col gap-4 overflow-auto border border-border bg-muted/75 p-4 max-[840px]:order-2" aria-label="Filter und Treffer">
         <section className="grid grid-cols-1 gap-2 3xs:grid-cols-2" aria-label="Layer">
